...
class ImageSet {
 public:
  ...
  void LoadAndAveragePSFs(class CachedImageSet& psfSet,
                          std::vector<aocommon::UVector<double>>& psfImages,
  ...
  void GetIntegratedPSF(double* dest,
  ...
  ImageSet& operator=(double val) {
    for (size_t i = 0; i != size(); ++i) assign(_images[i], val);
    return *this;
  }

  double* operator[](size_t index) { return _images[index]; }

  const double* operator[](size_t index) const { return _images[index]; }
  ...
  ImageSet& operator*=(double factor) {
    for (size_t i = 0; i != size(); ++i) multiply(_images[i], factor);
    return *this;
  }

  ImageSet& operator+=(const ImageSet& other) {
    for (size_t i = 0; i != size(); ++i) add(_images[i], other._images[i]);
    return *this;
  }
  ...
  void Set(size_t index, const double* rhs) { assign(_images[index], rhs); }
  ...
  static void CalculateDeconvolutionFrequencies(
      ...
      aocommon::UVector<double>& weights, size_t nDeconvolutionChannels);
 ...
 private:
  ...
  void assign(double* image, double value) const {
    for (size_t i = 0; i != _width * _height; ++i) image[i] = value;
  }

  void add(double* lhs, const double* rhs) const {
    for (size_t i = 0; i != _width * _height; ++i) lhs[i] += rhs[i];
  }

  void square(double* image) const {
    for (size_t i = 0; i != _width * _height; ++i) image[i] *= image[i];
  }

  void squareRoot(double* image) const {
    for (size_t i = 0; i != _width * _height; ++i) image[i] = sqrt(image[i]);
  }
  ...
  void addSquared(double* lhs, const double* rhs) const {
    for (size_t i = 0; i != _width * _height; ++i) lhs[i] += rhs[i] * rhs[i];
  }
  ...
  void multiply(double* image, double fact) const {
    if (fact != 1.0) {
      for (size_t i = 0; i != _width * _height; ++i) image[i] *= fact;
    }
  }
  ...
  static void copySmallerPart(const double* input, double* output, size_t x1,
  ...
  static void copyToLarger(double* to, size_t toX, size_t toY, size_t toWidth,
                           const double* from, size_t fromWidth,
                           size_t fromHeight) {
    for (size_t y = 0; y != fromHeight; ++y) {
      std::copy(from + y * fromWidth, from + (y + 1) * fromWidth,
  ...
  void getSquareIntegratedWithNormalChannels(double* dest,
