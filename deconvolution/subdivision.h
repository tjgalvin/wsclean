...
class Subdivision {
 public:
  Subdivision(size_t width, size_t height) : _width(width), _height(height) {}
  ...
  void GetBoundingMask(const double* subdivision, size_t subImgX,
    ...
    // created. The latter is necessary to prevent storing edge pixels in two
    // subimages. (While this could also be deduced by tracing the subdivision
    // paths, this was simpler -- at the cost of one extra array of booleans)
    ...
    size_t x2 = 0, y2 = 0;
    while (!stack.empty()) {
      size_t curX = stack.back().first, curY = stack.back().second;
      stack.pop_back();
      if (!visited[curX + curY * _width]) {
        ...
        if (subdivision[curX + curY * _width] == 0.0) {
          if (curX > 0) stack.emplace_back(curX - 1, curY);
          // When moving down / to the right, we do not allow to be on the
          // border, because the right border is excluded from the subdivision
          // edge. (note that the 'visited' mask prevents overlap, but this way
          // subimages consistenly include the left edge and exclude the right
          // edge as long as the edge is not surrounded on three sides, in which
          // case it is more or less random)
          if (curX < _width - 1 && subdivision[curX + 1 + curY * _width] == 0.0)
            stack.emplace_back(curX + 1, curY);
          if (curY > 0) stack.emplace_back(curX, curY - 1);
          if (curY < _height - 1 &&
              subdivision[curX + (curY + 1) * _width] == 0.0)
    ...
    if (_width % 2 == 0) {
      if (subWidth % 2 != 0) {
        if (subWidth + x >= _width) --x;
        ++subWidth;
      }
    }
    if (_height % 2 == 0) {
      if (subHeight % 2 != 0) {
        if (subHeight + y >= _height) --y;
        ++subHeight;
      }
    }
  }
