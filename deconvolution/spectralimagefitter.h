...
class SpectralImageFitter {
 public:
  SpectralImageFitter(size_t imgWidth, size_t imgHeight,
                      ImageBufferAllocator& allocator, SpectralFitter& fitter)
      : _width(imgWidth),
        _height(imgHeight),
  ...
  void PerformFit() {
    ...
    for (size_t px = 0; px != _width * _height; ++px) {
      ...
      for (size_t t = 0; t != terms.size(); ++t) _terms[t][px] = terms[t];
    }
  }
  ...
  void Interpolate(double* destination, double frequency) {
    aocommon::UVector<double> terms(_fitter.NTerms());
    for (size_t px = 0; px != _width * _height; ++px) {
      for (size_t t = 0; t != terms.size(); ++t) terms[t] = _terms[t][px];
 ...
 private:
  void clearTerms() {
    for (size_t i = 0; i != _terms.size(); ++i) _allocator.Free(_terms[i]);
    _terms.clear();
  }

  void clearImages() {
    for (size_t i = 0; i != _images.size(); ++i) _allocator.Free(_images[i]);
    _images.clear();
  }
