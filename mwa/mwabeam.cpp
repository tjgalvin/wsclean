...
void MWABeam::Make(PrimaryBeamImageSet& beamImages) {
  ...
  if (_width != _sampledWidth || _height != _sampledHeight) {
    ...
    for (size_t p = 0; p != 8; ++p) {
      ...
      memcpy(&beamImages[p][0], scratch.data(),
             sizeof(double) * _width * _height);
    }
  }
}

void MWABeam::makeBeamForMS(PrimaryBeamImageSet& beamImages,
                            MSProvider& msProvider, double centralFrequency) {
  /**
  ...
  Logger::Debug << "Making MWA beam for frequency " << centralFrequency * 1e-6
                << " MHz.\n";
  ...
  size_t intervalCount =
      (totalSeconds + _secondsBeforeBeamUpdate - 1) / _secondsBeforeBeamUpdate;
...
void MWABeam::makeBeamSnapshot(double** imgPtr, double frequency,
  ...
  for (size_t y = 0; y != _sampledHeight; ++y) {
    for (size_t x = 0; x != _sampledWidth; ++x) {
      double l, m, ra, dec;
      ImageCoordinates::XYToLM(x, y, _sPixelSizeX, _sPixelSizeY, _sampledWidth,
                               _sampledHeight, l, m);
      l += _phaseCentreDL;
      m += _phaseCentreDM;
