...
const double TileBeamBase<Implementation>::MWA_LATTITUDE =
    -26.703319;  // Array latitude. degrees North

template <typename Implementation>
const double TileBeamBase<Implementation>::MWA_LONGITUDE =
    116.67081;  // Array longitude. degrees East

template <typename Implementation>
const double TileBeamBase<Implementation>::MWA_HEIGHT =
    377.0;  // Array altitude. meters above sea level

template <typename Implementation>
void TileBeamBase<Implementation>::ArrayResponse(casacore::MEpoch& time,
                                                 casacore::MPosition& arrayPos,
                                                 double raRad, double decRad,
                                                 double frequencyHz,
  ...
  casacore::MDirection::Convert j2000ToHaDec(j2000Ref, hadecRef),
  ...
  ArrayResponse(raRad, decRad, j2000Ref, j2000ToHaDec, j2000ToAzelGeo,
                arrLatitude, zenithHa, zenithDec, frequencyHz, gain);
}

template <typename Implementation>
void TileBeamBase<Implementation>::ArrayResponse(
    double raRad, double decRad, const casacore::MDirection::Ref& j2000Ref,
    casacore::MDirection::Convert& j2000ToHaDec,
    casacore::MDirection::Convert& j2000ToAzelGeo, double arrLatitude,
    double haAntennaZenith, double decAntennaZenith, double frequencyHz,
    std::complex<double>* gain) {
  static const casacore::Unit radUnit("rad");
  casacore::MDirection imageDir(
      casacore::MVDirection(casacore::Quantity(raRad, radUnit),    // RA
  ...
  // std::cout << "ha=" << ha*180.0/M_PI << '-' << haAntennaZenith*180.0/M_PI <<
  // ", za=" << zenithDistance*180.0/M_PI << ", az=" << azimuth*180.0/M_PI <<
  // '\n';

  ArrayResponse(zenithDistance, azimuth, frequencyHz, ha, decRad,
                haAntennaZenith, decAntennaZenith, gain);
}

template <typename Implementation>
void TileBeamBase<Implementation>::PrecalculatePositionInfo(
    TileBeamBase::PrecalcPosInfo& posInfo, casacore::MEpoch& time,
    casacore::MPosition& arrayPos, double raRad, double decRad) {
  casacore::MeasFrame frame(arrayPos, time);
  const casacore::MDirection::Ref hadecRef(casacore::MDirection::HADEC, frame);
  const casacore::MDirection::Ref azelgeoRef(casacore::MDirection::AZELGEO,
                                             frame);
