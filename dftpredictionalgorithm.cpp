...
void DFTPredictionImage::Add(aocommon::PolarizationEnum polarization,
                             const double* image) {
  ...
  for (size_t y = 0; y != _height; ++y) {
    for (size_t x = 0; x != _width; ++x) {
      if (_images[0][index] != 0.0 || _images[1][index] != 0.0 ||
        ...
        double l, m;
        aocommon::ImageCoordinates::XYToLM(x, y, pixelSizeX, pixelSizeY, _width,
                                           _height, l, m);
        l += dl;
        m += dm;
        double ra, dec;
        aocommon::ImageCoordinates::LMToRaDec(l, m, phaseCentreRA,
        ...
        destination.AddComponent(
            DFTPredictionComponent(ra, dec, l, m, linear, channelCount));
      }
      ++index;
    }
  }
...
void DFTPredictionInput::ConvertApparentToAbsolute(
  ...
  for (std::vector<ComponentInfo>::iterator cInfo = compInfos.begin();
       cInfo != compInfos.end(); ++cInfo) {
  ...
  for (size_t i = 0; i != _components.size(); ++i) {
    ...
    for (size_t ch = 0; ch != band.ChannelCount(); ++ch) {
      ...
      if (ch == band.ChannelCount() / 2) {
        std::cout << RaDecCoord::RAToString(c.RA()) << " "
                  << RaDecCoord::DecToString(c.Dec()) << " :";
        for (size_t p = 0; p != 4; ++p)
      ...
      if (ch == band.ChannelCount() / 2)
        std::cout << c.LinearFlux(ch).ToString() << " (" << c.L() << ","
                  << c.M() << ")\n";
    }
  }
}

void DFTPredictionInput::InitializeFromModel(const Model& model,
  ...
  for (const ModelSource& s : model) {
    for (const ModelComponent& comp : s) {
      ...
      aocommon::ImageCoordinates::RaDecToLM(
          comp.PosRA(), comp.PosDec(), phaseCentreRA, phaseCentreDec, l, m);
      component.SetPosition(comp.PosRA(), comp.PosDec(), l, m);
      if (comp.Type() == ModelComponent::GaussianSource) {
        component.SetGaussianInfo(comp.PositionAngle(), comp.MajorAxis(),
                                  comp.MinorAxis());
      }
      ...
      for (size_t ch = 0; ch != band.ChannelCount(); ++ch) {
        ...
        double stokes[4];
        for (size_t p = 0; p != 4; ++p)
          stokes[p] = comp.SED().FluxAtFrequency(
              band.ChannelFrequency(ch),
...
void DFTPredictionAlgorithm::Predict(MC2x2& dest, double u, double v, double w,
  ...
  dest = MC2x2::Zero();
  MC2x2 single;
  for (DFTPredictionInput::const_iterator c = _input.begin(); c != _input.end();
       ++c) {
...
void DFTPredictionAlgorithm::UpdateBeam(LBeamEvaluator& beamEvaluator,
  ...
  for (DFTPredictionInput::iterator component = _input.begin();
       component != _input.end(); ++component) {
    LBeamEvaluator::PrecalcPosInfo posInfo;
