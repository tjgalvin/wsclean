...
LBeamEvaluator::LBeamEvaluator(casacore::MeasurementSet& ms) : _ms(ms) {
  ...
  casacore::ROScalarMeasColumn<casacore::MDirection> delayDirColumn(
      fieldTable,
  ...
  if (fieldTable.tableDesc().isColumn("LOFAR_TILE_BEAM_DIR")) {
    casacore::ROArrayMeasColumn<casacore::MDirection> tileBeamDirColumn(
  ...
  Logger::Debug << "Using delay direction: "
                << RaDecCoord::RaDecToString(
                       _tileBeamDir.getValue().getVector()[0],
                       _tileBeamDir.getValue().getVector()[1])
                << '\n';
...
void LBeamEvaluator::Evaluate(double ra, double dec, double frequency,
                              size_t antennaIndex, MC2x2& beamValues) {
  ...
  matrix22c_t gainMatrix =
      _stations[antennaIndex]->response(_timeAsDouble, frequency, itrfDirection,
                                        _subbandFrequency, _station0, _tile0);
...
void LBeamEvaluator::Evaluate(const LBeamEvaluator::PrecalcPosInfo& posInfo,
                              double frequency, size_t antennaIndex,
                              MC2x2& beamValues) {
  matrix22c_t gainMatrix = _stations[antennaIndex]->response(
      _timeAsDouble, frequency, posInfo.itrfDirection, _subbandFrequency,
      _station0, _tile0);
...
void LBeamEvaluator::EvaluateFullArray(
    const LBeamEvaluator::PrecalcPosInfo& posInfo, double frequency,
    MC2x2& beamValues) {
  beamValues = MC2x2::Zero();
  for (Station::Ptr s : _stations) {
    matrix22c_t gainMatrix =
        s->response(_timeAsDouble, frequency, posInfo.itrfDirection,
                    _subbandFrequency, _station0, _tile0);
...
void LBeamEvaluator::PrecalculatePositionInfo(
    LBeamEvaluator::PrecalcPosInfo& posInfo, double raRad, double decRad) {
  static const casacore::Unit radUnit("rad");
  casacore::MDirection imageDir(
      casacore::MVDirection(casacore::Quantity(raRad, radUnit),
...
void LBeamEvaluator::EvaluateFullCorrection(casacore::MeasurementSet& ms,
  ...
  for (size_t i = 0; i != nrow; ++i) {
    ...
    if (time.getValue().get() != evaluator.Time().getValue().get()) {
      ...
      for (size_t ch = 0; ch != band.ChannelCount(); ++ch) {
        MC2x2 timeStepValue;
        evaluator.EvaluateFullArray(posInfo, band.ChannelFrequency(ch),
                                    timeStepValue);
