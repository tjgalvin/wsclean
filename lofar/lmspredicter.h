...
class LMSPredicter {
 public:
  ...
  bool GetNextRow(RowData& data) { return _bufferedOutputLane.read(data); }

  void FinishRow(RowData& data) { _bufferedBufferLane.write(data); }
  ...
  void SetUseModelColumn(bool useModelColumn) {
    _useModelColumn = useModelColumn;
  }
