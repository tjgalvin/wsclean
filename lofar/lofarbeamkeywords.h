...
class LOFARBeamKeywords {
 public:
  static bool GetPreappliedBeamDirection(
      casacore::MeasurementSet& ms, const std::string& dataColumnName,
      bool useDifferentialBeam, casacore::MDirection& preappliedBeamDir) {
    ...
    if (useDifferentialBeam) {
      if (wasBeamApplied)
        Logger::Warn << "* This measurement set has keywords specifying the "
                        "pre-applied beam mode & direction:"
                        "  it is therefore unnecessary to specify "
                        "'-use-differential-lofar-beam'.\n";
      else
        Logger::Warn << "!!!\n"
                        "!!! The differential beam is being applied, but the "
                        "measurement specifies that no\n"
                        "!!! beam was pre-applied. Either an old version of "
                        "DP3 was used, or you are applying\n"
                        "!!! the wrong beam!\n"
                        "!!!\n";
    }
    if (wasBeamApplied || useDifferentialBeam) {
      ...
      Logger::Debug << "Applying differential beam from direction "
