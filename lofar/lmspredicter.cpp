...
void LMSPredicter::InitializeInput(const Model& model) {
  ...
  casacore::MDirection::ROScalarColumn refDirColumn(
...
void LMSPredicter::clearBuffers() {
  for (aocommon::MC2x2* buffer : _buffers) delete[] buffer;
}
...
void LMSPredicter::ReadThreadFunc() {
  ...
  if (!_useModelColumn) {
    if (_dftInput.ComponentCount() == 0) actualThreadCount = 1;
  ...
  for (size_t rowIndex = _startRow; rowIndex != _endRow; ++rowIndex) {
    ...
    if (a1 != a2) {
      ...
      if (!_useModelColumn && _applyBeam &&
        ...
        // Put all threads on wait, then update beam values, then restart
        // threads.
        _workLane.write_end();
        _barrier.wait();
...
void LMSPredicter::PredictThreadFunc() {
  ...
  while (_workLane.read(rowData)) {
    ...
    for (size_t ch = _startChannel; ch != _endChannel; ++ch) {
      ...
      _predicter->Predict(*valIter, rowData.u / lambda, rowData.v / lambda,
                          rowData.w / lambda, ch, rowData.a1, rowData.a2);
      ++valIter;
    }
    _outputLane.write(rowData);
  }
