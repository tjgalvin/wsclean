...
class SkyAreaElement {
 public:
  ...
  bool IsIn(NumType ra, NumType dec) const {
    switch (_type) {
      case Circle:
        return aocommon::ImageCoordinates::AngularDistance<NumType>(
 ...
 public:
  ...
  void FindAreasInImage(std::vector<const SkyArea *> &foundAreas, size_t x,
    ...
    aocommon::ImageCoordinates::XYToLM<long double>(
        x, y, _pixelSizeX, _pixelSizeY, _imageWidth, _imageHeight, l, m);
    aocommon::ImageCoordinates::LMToRaDec<long double>(l, m, _imageRA,
                                                       _imageDec, ra, dec);
