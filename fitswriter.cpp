...
void FitsWriter::writeHeaders(fitsfile*& fptr,
...
void FitsWriter::writeHeaders(
    fitsfile*& fptr, const std::string& filename,
  ...
  switch (_unit) {
    ...
    case Jansky:
      ...
      break;
    case Kelvin:
      fits_write_key(fptr, TSTRING, "BUNIT", (void*)"K", "Units are in Kelvin",
                     &status);
      checkStatus(status, filename);
      break;
    case MilliKelvin:
  ...
  if (_isUV) {
    ...
    checkStatus(status, filename);
    fits_write_key(fptr, TDOUBLE, "CRPIX1", (void*)&centrePixelX, "", &status);
    checkStatus(status, filename);
    ...
    checkStatus(status, filename);
    fits_write_key(fptr, TDOUBLE, "CRPIX2", (void*)&centrePixelY, "", &status);
    checkStatus(status, filename);
  ...
  for (size_t i = 0; i != extraDimensions.size(); ++i) {
    ...
    switch (extraDimensions[i].type) {
      ...
      case AntennaDimension:
        fits_write_key(fptr, TSTRING, ctypeDim, (void*)"ANTENNA", "", &status);
        checkStatus(status, filename);
        fits_write_key(fptr, TDOUBLE, crpixDim, (void*)&one, "", &status);
        checkStatus(status, filename);
...
void FitsWriter::writeImage(fitsfile* fptr, const std::string& filename,
...
void FitsWriter::WriteMask(const std::string& filename,
...
template <typename NumType>
void FitsWriter::Write(const std::string& filename,
                       const NumType* image) const {
  fitsfile* fptr;
...
template void FitsWriter::Write<long double>(const std::string& filename,
                                             const long double* image) const;
template void FitsWriter::Write<double>(const std::string& filename,
                                        const double* image) const;
template void FitsWriter::Write<float>(const std::string& filename,
...
void FitsWriter::julianDateToYMD(double jd, int& year, int& month,
                                 int& day) const {
  int z = jd + 0.5;
...
void FitsWriter::MJDToHMS(double mjd, int& hour, int& minutes, int& seconds,
                          int& deciSec) {
...
void FitsWriter::CopyDoubleKeywordIfExists(FitsReader& reader,
...
void FitsWriter::CopyStringKeywordIfExists(FitsReader& reader,
                                           const char* keywordName) {
  std::string v;
