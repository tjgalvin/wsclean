...
class DFTAntennaInfo {
 public:
  const aocommon::MC2x2& BeamValue(size_t channelIndex) const {
    return _beamValuesPerChannel[channelIndex];
  }
  aocommon::MC2x2& BeamValue(size_t channelIndex) {
    return _beamValuesPerChannel[channelIndex];
  }

  std::vector<aocommon::MC2x2>::iterator begin() {
    return _beamValuesPerChannel.begin();
  }
  std::vector<aocommon::MC2x2>::iterator end() {
    return _beamValuesPerChannel.end();
  }
...
class DFTPredictionComponent {
 public:
  ...
  DFTPredictionComponent(double ra, double dec, double l, double m,
                         std::complex<double> fluxLinear[4],
                         size_t channelCount)
      : _ra(ra),
        _dec(dec),
        _l(l),
  ...
  aocommon::MC2x2& LinearFlux(size_t channelIndex) {
    return _flux[channelIndex];
  }
  const aocommon::MC2x2& LinearFlux(size_t channelIndex) const {
    return _flux[channelIndex];
  }
  ...
  void InitializeBeamBuffers(size_t antennaCount, size_t channelCount) {
    ...
    for (std::vector<DFTAntennaInfo>::iterator a =
             _beamValuesPerAntenna.begin();
         a != _beamValuesPerAntenna.end(); ++a)
 ...
 private:
  void initializeGaussian(double positionAngle, double majorAxis,
    ...
    double paSin = std::sin(positionAngle + 0.5 * M_PI),
...
class DFTPredictionInput {
 public:
  ...
  DFTPredictionInput() {}
  void InitializeFromModel(const class Model& model, long double phaseCentreRA,
  ...
  void SetUnitaryBeam() {
    for (iterator c = begin(); c != end(); ++c) c->SetUnitaryBeam();
  }
...
class DFTPredictionImage {
 public:
  ...
  void Add(aocommon::PolarizationEnum polarization, const double* real,
...
class DFTPredictionAlgorithm {
 public:
  DFTPredictionAlgorithm(DFTPredictionInput& input,
                         const aocommon::BandData& band)
      : _input(input), _band(band), _hasBeam(false) {}

  void Predict(aocommon::MC2x2& dest, double u, double v, double w,
               size_t channelIndex, size_t a1, size_t a2);

  void UpdateBeam(LBeamEvaluator& beamEvaluator, size_t startChannel,
                  size_t endChannel);
