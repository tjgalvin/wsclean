...
class WGriddingGridder_Simple
{
	...
	public:
		...
		void InitializeInversion();
                 ...
                 */
                void AddInversionData(size_t nrows, size_t nchan,
		...
		void InitializePrediction(std::vector<float> &&image);
                 ...
                 *        visibility(row, chan) := vis[row*nchan + chan]
                 */
                void PredictVisibilities(size_t nrows, size_t nchan,
                  const double *uvw, const double *freq,
...
WGriddingGridder_Simple gridder(width, height, pixelSizeX, pixelSizeY, nthreads, 1e-5);
