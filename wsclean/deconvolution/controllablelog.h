...
{
public:
	ControllableLog(std::mutex* mutex) : _mutex(mutex), _atNewLine(true), _isMuted(false), _isActive(true)
	{ }
...
public:
	...
	ControllableLog& operator[](size_t index)
	{
		return _logs[index];
	}
	...
	void unmuteMostCentral()
	{
		...
		for(size_t y=0; y!=_nVertical; ++y)
		{
			for(size_t x=0; x!=_nHorizontal; ++x)
			{
				...
				if(_logs[index].IsActive())
				{
					int dx = int(x)-int(_nHorizontal)/2, dy = int(y)-int(_nVertical)/2;
		...
		if(found)
		{
			...
			if(index != unmutedLog)
			{
				_logs[index].SetOutputOnce("Switching to output of subimage " + std::to_string(index) + "\n");
			}
		}
	}
