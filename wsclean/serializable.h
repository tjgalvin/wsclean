...
class Serializable
{
	public:
		...
		static void SerializeToFloatC(std::ostream& stream, std::complex<float> value)
		{
			stream.write(reinterpret_cast<char *>(&value), sizeof(value));
		}
		...
		template<typename T>
		static void SerializeVectorUInt64(std::ostream& stream, const std::vector<T>& values)
		{
			uint64_t size = values.size();
			SerializeToUInt64(stream, size);
			for(const T val : values)
				SerializeToUInt64(stream, val);
		}
		
		template<typename T>
		static void UnserializeVectorUInt64(std::istream& stream, std::vector<T>& values)
		{
			uint64_t size = UnserializeUInt64(stream);
			values.resize(size);
			for(size_t i=0; i!=size; ++i)
				values[i] = UnserializeUInt64(stream);
		}
		
		static void SerializeVectorFloat(std::ostream& stream, const std::vector<float>& values)
		{
		...
		static std::vector<float> UnserializeVectorFloat(std::istream& stream)
		{
			...
			for(float& val : values)
				val = UnserializeFloat(stream);
			return values;
		}
		
		static void SerializeVectorDouble(std::ostream& stream, const std::vector<double>& values)
		{
			SerializeToUInt64(stream, values.size());
			for(const double& val : values)
				SerializeToDouble(stream, val);
		}
		...
		static std::vector<double> UnserializeVectorDouble(std::istream& stream)
		{
			...
			for(double& val : values)
				val = UnserializeDouble(stream);
			return values;
		}
		
		static void SerializeVectorFloatC(std::ostream& stream, const std::vector<std::complex<float>>& values)
		{
			SerializeToUInt64(stream, values.size());
		...
		static std::vector<std::complex<float>> UnserializeVectorFloatC(std::istream& stream)
		{
			std::vector<std::complex<float>> values(UnserializeUInt64(stream));
			for(std::complex<float>& val : values)
				val = UnserializeFloatC(stream);
			return values;
		}
		
		template<typename T>
		static void SerializePtr(std::ostream& stream, const std::unique_ptr<T>& ptr)
		{
			if(ptr)
			{
		...
		template<typename T>
		static void SerializePtr(std::ostream& stream, const std::shared_ptr<T>& ptr)
		{
			if(ptr)
			{
				Serializable::SerializeToBool(stream, true);
