...
public:
	...
	{ 
		...
		return ((l&32) == 0) ? (
				...
				std::complex<double>(_data[l & (~16)], -_data[(l & (~16)) + 1])
			) : (
				_data[l & (~32)]
			);
	}
	...
	double Norm() const
	{
		return
			// diagonal
			_data[0]*_data[0] + _data[3]*_data[3] +
			_data[8]*_data[8] + _data[15]*_data[15] +
			// lower half x 2
			2.0 * (
				std::norm(toComplex(1)) + std::norm(toComplex(4)) +
