...
class ImageBufferAllocator {
 public:
  ...
  class TPtr {
   public:
    ...
    TPtr() : _data(nullptr), _allocator(nullptr) {}
    TPtr(std::nullptr_t) : _data(nullptr), _allocator(nullptr) {}
  ...
  class CPtr {
   public:
    friend class ImageBufferAllocator;
    /** Construct empty pointer */
    CPtr() : _data(nullptr), _allocator(nullptr) {}
    CPtr(std::nullptr_t) : _data(nullptr), _allocator(nullptr) {}
    /** Construct initialized pointer
     * @param data databuffer
     * @param allocator corresponding allocator
     */
  ...
  ImageBufferAllocator(bool direct)
      : _buffers(),
        _nReal(0),
        _nComplex(0),
        _nRealMax(0),
  ...
  void Free(float* buffer) { Free(reinterpret_cast<double*>(buffer)); }
 ...
 private:
  ...
  double* directAllocate(size_t size) {
    double* ptr;
    int errVal = posix_memalign(reinterpret_cast<void**>(&ptr), sizeof(double),
                                size * sizeof(double));
    if (errVal != 0) {
  ...
  void clear() {
    ...
    for (Buffer& buffer : _buffers) {
      ...
      // diagnose it as well.
      if (!buffer.isFirstHalfUsed && !buffer.isSecondHalfUsed) free(buffer.ptr);
    }
    if (usedCount != 0) {
      std::cerr << usedCount
...
template <>
inline ImageBufferAllocator::TPtr<double> ImageBufferAllocator::AllocateTPtr(
    size_t size) {
  return AllocatePtr(size);
}

template <>
inline ImageBufferAllocator::TPtr<float> ImageBufferAllocator::AllocateTPtr(
    size_t size) {
  return TPtr<float>(reinterpret_cast<float*>(Allocate((size + 1) / 2)), *this);
}

template <>
inline ImageBufferAllocator::CPtr<double> ImageBufferAllocator::AllocateCPtr(
    size_t size) {
  return CPtr<double>(AllocateComplex(size), *this);
}

template <>
inline ImageBufferAllocator::CPtr<float> ImageBufferAllocator::AllocateCPtr(
    size_t size) {
  return CPtr<float>(
      reinterpret_cast<std::complex<float>*>(AllocateComplex((size + 1) / 2)),
      *this);
}
