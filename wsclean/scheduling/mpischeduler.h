...
public:
	...
	void Run(GriddingTask& task, std::function<void (GriddingResult &)> finishCallback) override;
...
private:
	...
	int findAndSetNodeState(MPIScheduler::NodeState currentState, std::pair<MPIScheduler::NodeState, std::function<void (GriddingResult &)>> newState);
