...
public:
	...
	{
		if(list.size() != 16)
			throw std::runtime_error("Matrix4x4 needs to be initialized with 16 items");
		size_t index = 0;
	...
	Matrix4x4 operator+(const Matrix4x4& rhs) const
	{
		...
		for(size_t i=0; i!=16; ++i)
			result[i] = this->_data[i] + rhs._data[i];
		return result;
	}
	...
	{
		Vector4 v(
			_data[0]*rhs[0], _data[4]*rhs[0], _data[8]*rhs[0], _data[12]*rhs[0]
		);
