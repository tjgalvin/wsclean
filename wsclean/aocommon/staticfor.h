...
namespace ao {
  ...
  public:
     ...
     *   void loopFunction(size_t chunkStart, size_t chunkEnd);
     */
    void Run(Iter start, Iter end, size_t nChunks, std::function<void(Iter, Iter)> function)
    {
      if(end == start+1)
      {
        function(start, end);
  ...
  private:
    ...
    void loop()
    {
      ...
      while(next(chunk)) {
        ...
        Iter chunkEnd = _iterStart + (_iterEnd - _iterStart) * (chunk+1) / _nChunks;
  ...
  public:
     ...
     *   void loopFunction(size_t chunkStart, size_t chunkEnd);
     */
    void Run(Iter start, Iter end, std::function<void(Iter, Iter, size_t)> function)
    {
      Run(start, end, _nThreads, std::move(function));
    }
     ...
     *   void loopFunction(size_t chunkStart, size_t chunkEnd);
     */
    void Run(Iter start, Iter end, size_t nChunks, std::function<void(Iter, Iter, size_t)> function)
    {
      if(end == start+1)
      {
