...
DishATerm::DishATerm(casacore::MeasurementSet& ms, const CoordinateSystem& coordinateSystem) :
	_width(coordinateSystem.width), _height(coordinateSystem.height),
	_phaseCentreRA(coordinateSystem.ra), _phaseCentreDec(coordinateSystem.dec),
	_dl(coordinateSystem.dl), _dm(coordinateSystem.dm),
	_phaseCentreDL(coordinateSystem.phaseCentreDL), _phaseCentreDM(coordinateSystem.phaseCentreDM),
	...
	_cachedFrequency(0.0)
{
	...
	casacore::ArrayColumn<double> pointingDirCol(fieldTable, casacore::MSField::columnName(casacore::MSField::DELAY_DIR));
...
bool DishATerm::calculateBeam(std::complex<float>* buffer, double, double frequency, size_t fieldId)
{
	if(fieldId == _cachedFieldId && _cachedFrequency == frequency)
	{
		...
		Logger::Debug << "Calculating VLA beam for field " << fieldId << ", frequency " << frequency*1e-6 << " MHz.\n";
		double
			pDirRA = _fieldPointing[fieldId].first,
		...
		{
			std::copy_n(buffer, _width*_height*4, buffer + i*_width*_height*4);
		}
