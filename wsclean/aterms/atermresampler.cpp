...
ATermResampler::ATermResampler(const ATermBase::CoordinateSystem& coordinateSystem) :
	_coordinateSystem(coordinateSystem),
	_allocatedWidth(coordinateSystem.maxSupport), _allocatedHeight(coordinateSystem.maxSupport),
	_downsample(true),
...
void ATermResampler::ReadAndResample(FitsReader& reader, size_t fileIndex, aocommon::UVector<double>& scratch, aocommon::UVector<double>& output)
{
	...
	{
		_resampler.reset(new FFTResampler(_allocatedWidth, _allocatedHeight, _coordinateSystem.width, _coordinateSystem.height, 1, false));
...
void ATermResampler::regrid(const FitsReader& reader, double* dest, const double* source)
{
	 ...
	 */
	bool samePlane = inPhaseCentreRA == _coordinateSystem.ra && inPhaseCentreDec == _coordinateSystem.dec;
	...
	for(size_t y=0; y!=outWidth; ++y)
	{
		...
		{
			double l, m;
			aocommon::ImageCoordinates::XYToLM(x, y, atermDL, atermDM, outWidth, outWidth, l, m);
			l += _coordinateSystem.phaseCentreDL;
			m += _coordinateSystem.phaseCentreDM;
			if(!samePlane)
			{
				double pixra, pixdec;
				aocommon::ImageCoordinates::LMToRaDec(l, m, _coordinateSystem.ra, _coordinateSystem.dec, pixra, pixdec);
				aocommon::ImageCoordinates::RaDecToLM(pixra, pixdec, inPhaseCentreRA, inPhaseCentreDec, l, m);
			}
			l -= inPhaseCentreDL;
			m -= inPhaseCentreDM;
			int inX, inY;
			aocommon::ImageCoordinates::LMToXY(l, m, inPixelSizeX, inPixelSizeY, inWidth, inHeight, inX, inY);
