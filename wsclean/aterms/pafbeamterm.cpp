...
bool PAFBeamTerm::Calculate(std::complex<float>* buffer, double time, double frequency, size_t /*fieldId*/, const double* /*uvwInM*/)
{
...
void PAFBeamTerm::Open(const std::string& filenameTemplate, const std::vector<std::string>& antennaMap, const std::string& beamName, double beamRA, double beamDec)
{
	...
	for(size_t antenna=0; antenna!=_nAntenna; ++antenna)
	{
		std::string filename = boost::algorithm::replace_all_copy(filenameTemplate, "$ANT", antennaMap[antenna]);
		...
		if(antenna == 0)
		{
			...
			if(_freq0 != _readers.back().FrequencyDimensionStart() ||
				_dfreq != _readers.back().FrequencyDimensionIncr() ||
			_nFrequencies != _readers.back().NFrequencies())
				throw std::runtime_error("All antenna fits files should have the same frequency axis");
		}
	}
}
