 ...
 * 
 * This is used in IDGMSGridder to be able to get the UVWs for calculating the a-terms.
 */
...
public:
	...
	size_t RowId() const override
	{
		return _buffer[_bufferPosition].rowId;
	}
	...
	{
		std::copy(_buffer[_bufferPosition].data.begin(), _buffer[_bufferPosition].data.end(), buffer);
	}
	
	void ReadModel(std::complex<float>* buffer) override
	{
		std::copy(_buffer[_bufferPosition].model.begin(), _buffer[_bufferPosition].model.end(), buffer);
	}
	...
	{ 
		std::copy(_buffer[_bufferPosition].weights.begin(), _buffer[_bufferPosition].weights.end(), buffer);
	}
	...
	/**
	 * Returns an Array containing the uvws for baselines (antenna1, antenna2) that have
	...
	{ return _msProvider->NChannels(); }
	
	size_t NAntennas() override
	{ return _msProvider->NAntennas(); }
	
	size_t NPolarizations() override
	{ return _msProvider->NPolarizations(); }
