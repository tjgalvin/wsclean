...
public:
	...
	SerialOStream& Bool(bool value)
	{
		return UInt8(value ? 1 : 0);
	}
		
	SerialOStream& Float(float value)
	{
		return write(value);
	}
	
	SerialOStream& Double(double value)
	{
		return write(value);
	}
	
	SerialOStream& LDouble(long double value)
	{
		return write(value);
	}
	
	SerialOStream& CFloat(std::complex<float> value)
	{
		return write(value);
	}
	
	SerialOStream& CDouble(std::complex<double> value)
	{
		return write(value);
	}
	...
	SerialOStream& VectorCDouble(const std::vector<std::complex<double>>& values)
	{
		return writeVector(values);
	}
...
private:
	...
	SerialOStream& writeVector(const std::vector<T>& values)
	{
		...
		const unsigned char* valuePtr = reinterpret_cast<const unsigned char*>(values.data());
