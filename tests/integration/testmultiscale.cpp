...
int main(int argc, char* argv[])
{
	if(argc <= 1)
	{
		std::cout << "Syntax: testmultiscale <freqcount> <niter> {<image> <psf>} [..]\n";
	}
	...
	ImageSet
		dirtySet(&table, settings),
		modelSet(&table, settings);
	...
	MultiScaleAlgorithm multiscale(fftwManager, beamSize, pixelScaleX, pixelScaleY);
	...
	/*if(hasInitialModel)
	{
		for(size_t i=0; i!=freqCount; ++i)
		{
			...
			FFTConvolver::PrepareKernel(psfKernel.data(), psfs[i].data(), width, height);
		
			// Calculate: residual = dirty - model (x) psf
			aocommon::UVector<double> tmp(modelSet[i], modelSet[i]+width*height);
			FFTConvolver::ConvolveSameSize(tmp.data(), psfKernel.data(), width, height);
	...
	multiscale.ExecuteMajorIteration(dirtySet, modelSet, psfVec, width, height, reachedMajorThreshold);
	
	std::cout << "Time taken: " << watch.ToString() << ", iterations per second: " << double(nIter) / watch.Seconds() << '\n';
