...
#define MWA_MS "../test_data/MWA_MOCK.ms/"

BOOST_AUTO_TEST_SUITE(vela_deconvolution)

BOOST_AUTO_TEST_CASE(wstacking) {
  if (boost::filesystem::is_directory(MWA_MS)) {
    WSClean wsclean;
    ...
    for (size_t ch = 0; ch != kNChan + 1; ++ch) {
      std::string chStr =
          (ch == kNChan) ? ("MFS") : ("000" + std::to_string(ch));
    ...
    BOOST_CHECK_EQUAL(residualReader.ImageWidth(), 1024);
    BOOST_CHECK_EQUAL(residualReader.ImageHeight(), 1024);
