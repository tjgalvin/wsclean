...
BOOST_AUTO_TEST_CASE(sed)
{
	...
	std::cout << "3c196 fit: " << terms3c196[0] << " " << terms3c196[1] << " " << terms3c196[2] << '\n';
	...
	{
		std::cout << data5[i] << "->" << NonLinearPowerLawFitter::Evaluate(freqs5[i], negTerms, refFreq5) << ' ';
	}
	std::cout << '\n';
	...
	for(size_t i=0; i!=4; ++i)
		std::cout << '[' << i << "]=" << NonLinearPowerLawFitter::Evaluate(i+1, terms6) << ' ';
	std::cout << '\n';
	...
	for(size_t i=0; i!=4; ++i)
	{
		std::cout << data7[i] << "->" << NonLinearPowerLawFitter::Evaluate(freqs7[i], terms7, refFreq7) << ' ';
	}
	std::cout << '\n';
