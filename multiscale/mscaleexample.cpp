...
double rms(const double* image, size_t size) {
  double sum = 0.0;
  for (const double* i = image; i != image + size; ++i) sum += (*i) * (*i);
...
int main(int argc, char* argv[]) {
  if (argc != 3) {
    ...
    for (size_t i = 0; i != width * height; ++i) model[i] = 0.0;

    double gain = 0.1;
    bool allowNegativeComponents = true;
    double borderRatio = 0.05;

    MultiScaleAlgorithm algorithm(allocator, beamScale, gain,
                                  allowNegativeComponents, borderRatio);
    algorithm.ExecuteMajorIteration(image.data(), model.data(), psf.data(),
                                    width, height);
