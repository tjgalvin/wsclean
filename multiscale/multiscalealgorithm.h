...
class MultiScaleAlgorithm : public DeconvolutionAlgorithm {
 public:
  ...
  void SetMultiscaleScaleBias(double bias) { _multiscaleScaleBias = bias; }
  void SetMultiscaleGain(double gain) { _multiscaleGain = gain; }
  void SetConvolutionPadding(double padding) { _convolutionPadding = padding; }
 ...
 private:
  ...
  void convolvePSFs(std::unique_ptr<Image[]>& convolvedPSFs, const double* psf,
                    double* tmp, bool isIntegrated);
  void findActiveScaleConvolvedMaxima(const ImageSet& imageSet,
                                      double* integratedScratch,
                                      double* scratch, bool reportRMS,
                                      ThreadedDeconvolutionTools* tools);
  bool selectMaximumScale(size_t& scaleWithPeak);
  void activateScales(size_t scaleWithLastPeak);
  void measureComponentValues(aocommon::UVector<double>& componentValues,
                              size_t scaleIndex, ImageSet& imageSet);
  void addComponentToModel(double* model, size_t scaleWithPeak,
                           double componentValue);

  void findPeakDirect(const double* image, double* scratch, size_t scaleIndex);

  double* getConvolvedPSF(
